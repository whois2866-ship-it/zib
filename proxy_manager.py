...
class ProxyManager:
    ...
    def _load(self, filepath: str):
        try:
            ...
            log.info("Loaded %d proxies from %s", len(self.proxies), filepath)
        except FileNotFoundError:
            if Config.PROXY_URL:
                self.proxies = [{"server": Config.PROXY_URL}]
                log.info("Using single proxy from .env")
            else:
                log.info("No proxies configured. Running without proxy.")
    ...
    def remove_current(self):
        ...
        if self.proxies and self.index > 0:
            ...
            log.info("Removed dead proxy. %d remaining.", len(self.proxies))
