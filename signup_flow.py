...
async def select_birthday(page: Page):
    ...
    try:
        ...
        print(f"  Birthday set: {month}/{day}/{year}")
        return True
    except Exception:
        pass
    ...
    try:
        ...
        print(f"  Birthday set (fallback): {month}/{day}/{year}")
        return True
    except Exception as e:
        print(f"  Birthday selection failed: {e}")
        return False
    ...
    for sel in email_selectors:
        try:
            ...
            if await el.count() > 0:
                ...
                print(f"  Email entered: {email_addr}")
                break
        except Exception:
            continue
...
async def handle_captcha(page: Page, solver: CaptchaSolver) -> bool:
    ...
    try:
        ...
        if await fc_element.count() > 0:
            print("  FunCaptcha detected, solving via API...")
...
async def submit_and_verify(page: Page, email_addr: str, postal: PostalEmailHandler) -> bool:
    ...
    if not code_input:
        print("  No verification code input found — might have succeeded or failed")
        return False
    ...
    if await send_code_btn.count() > 0:
        ...
        print("  Sent verification code request")
        await asyncio.sleep(2)
 
    # Wait for verification code from Postal
    print("  Waiting for verification code via Postal...")
...
async def create_account(
    ...
    try:
        print(f"\n{'='*50}")
        print(f"Creating account with: {email_addr}")
        print(f"{'='*50}")
        ...
        if not await select_birthday(page):
            print("  FAILED: Could not set birthday")
            return None
        ...
        if not verified:
            print("  FAILED: Email verification failed")
            return None
        ...
        if success:
            ...
            print(f"\n  SUCCESS! Account created: {email_addr} / {username}")
        else:
